...
int main(int argc, char* argv[])
{
    ...
    //     // printf("Use arg: %s\n", argv[1]);
    // }
    int input_mode = 1; // 0 = from file, 1 = from stdin
    ...
    oj:;
        ...
        for (int i = 0; i < 1000010; i++) {
            if (!b2) {
                ...
                action_taken = runGameStep(&ctx, findBlock(b1), input_mode ? (i >= change_to_2 ? 1 : 3) : 2);
